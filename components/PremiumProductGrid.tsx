...
import { ShoppingCart, Zap, Heart, Clock, CreditCard } from 'lucide-react';
...
interface Product {
  ...
  inventory?: number; // Quantiy in stock
}
...
function PremiumProductGrid({ products }: PremiumProductGridProps) {
  ...
  const handleAddToCart = (product: Product) => {
    addItem({ ...product, rating: 4.5, image: product.image || '' } as any);
        ...
        return (
          ...
          >
            {product.id ? (
                ...
                <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 transition-all duration-300 shadow-lg group-hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] group-hover:border-orange-200 h-full flex flex-col">
                  ...
                  <div className="relative aspect-square overflow-hidden bg-gray-50">
                    ...
                    >
                      ...
                      >
                        <Heart
                          className={`w-5 h-5 transition-colors duration-300 ${isWishlisted ? 'fill-red-500 text-red-500' : 'text-slate-400 group-hover/heart:text-slate-600'
                            }`}
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex items-center gap-1">
                        {getStarRating(rating).map((filled, i) => (
                          <svg
                            key={i}
                            className={`w-4 h-4 ${filled ? 'fill-current text-orange-500' : 'fill-current text-gray-300'
                              }`}
                            viewBox="0 0 20 20"
                          >
                      ...
                      {/* Fast Delivery Info & Low Stock Warning */}
                      {product.stockStatus === 'in-stock' && (
                        <div className="flex items-center gap-1.5 pt-1">
                          {product.inventory && product.inventory < 5 ? (
                            <div className="flex items-center gap-1.5 animate-pulse">
                              <Clock className="w-3 h-3 text-red-500 fill-current" />
                              <span className="text-[10px] font-bold text-red-600">
                                {product.inventory === 1 ? 'Last 1 left!' : `Only ${product.inventory} left!`}
                              </span>
                            </div>
                          ) : (
                            <>
                    ...
                    <div className="flex gap-2 pt-2">
                      <MagneticButton className="flex-1">
                        ...
                        >
                          ...
                          <span className="text-[10px] uppercase tracking-wider relative z-10">{t('product', 'addToCart')}</span>
                        </button>
                      </MagneticButton>
                      ...
                      <MagneticButton className="flex-1">
                        <button
                          ...
                          className="relative w-full py-3 bg-slate-900 hover:bg-black text-white font-bold rounded-xl shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group/btn overflow-hidden"
                        >
                          <div className="absolute inset-0 translate-x-[-100%] group-hover/btn:translate-x-[100%] transition-transform duration-700 bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-12" />
                          <CreditCard className="w-4 h-4 transition-transform group-hover/btn:scale-110 relative z-10" strokeWidth={1.2} />
                          <span className="text-[10px] uppercase tracking-wider relative z-10">Авах</span>
                        </button>
                      </MagneticButton>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 h-full flex flex-col opacity-60 pointer-events-none">
